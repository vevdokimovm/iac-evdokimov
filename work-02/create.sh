...
# ---- параметры варианта ----
PREFIX=evdokimov-11          # префикс имён ресурсов
ZONE_A=ru-central1-b         # зона A
ZONE_B=ru-central1-d         # зона B
CIDR_A=10.21.1.0/24          # подсеть в зоне A
CIDR_B=10.21.2.0/24          # подсеть в зоне B
APP_PORT=8033                # порт, на котором отвечает nginx
GREETING=devlab              # слово из варианта, оно же на странице
VM_COUNT="${1:-2}"           # число машин в группе — аргумент 1, по умолчанию из варианта
DISK_SIZE="${2:-10}"         # дополнительный диск, ГБ — аргумент 2, по умолчанию из варианта
BOOT_SIZE=20                 # загрузочный диск, ГБ — из варианта
IMAGE_FAMILY=ubuntu-2404-lts # образ машин, одинаковый у всех вариантов

[[ "$VM_COUNT" =~ ^[1-9][0-9]*$ ]] || { echo "VM_COUNT должно быть целым > 0: $VM_COUNT" >&2; exit 1; }
[[ "$DISK_SIZE" =~ ^[1-9][0-9]*$ ]] || { echo "DISK_SIZE должно быть целым > 0: $DISK_SIZE" >&2; exit 1; }
